...
Flag cache_access(Cache *c, Addr lineaddr, uns is_write, uns core_id){
  Flag outcome=MISS;
    ...
    uns64 num=c->num_sets;
    
    uns temp = lineaddr & (num-1);
    Addr tag = lineaddr / num;
    
    for(ii = 0; ii < c->num_ways; ii++){
        
        if(c->sets[temp].line[ii].tag == tag && c->sets[temp].line[ii].valid){
            outcome = HIT;
            c->sets[temp].line[ii].last_access_time = cycle;
            
            if(is_write){
                c->sets[temp].line[ii].dirty = TRUE;
                c->stat_write_access++;
            }
            else{
                c->stat_read_access++;
...
void cache_install(Cache *c, Addr lineaddr, uns is_write, uns core_id){
  ...
  // Your Code Goes Here
    ...
    vic_line = cache_find_victim(c, temp, core_id); 
    
    c->last_evicted_line = c->sets[temp].line[vic_line];
    if(c->last_evicted_line.dirty)
        c->stat_dirty_evicts++;  
    
    c->sets[temp].line[vic_line].valid = 1;
    c->sets[temp].line[vic_line].tag = tag;
    c->sets[temp].line[vic_line].core_id = core_id;
    c->sets[temp].line[vic_line].last_access_time = cycle;
    
    if(!is_write){
        c->sets[temp].line[vic_line].dirty = 0;
        c->stat_read_miss++;
        c->stat_read_access++;

    }
    else{
        c->sets[temp].line[vic_line].dirty = 1;
        c->stat_write_miss++;
        c->stat_write_access++;
...
uns victim=0;
    ...
    uns ii, lru = c->num_ways, temp = 0xFFFFFFFF;
    ...
    switch(c->repl_policy){
        ...
        case 0:
            ...
            for(ii = 0; ii < c->num_ways; ii++){
                ...
                if(temp > c->sets[set_index].line[ii].last_access_time){

...
uns64 memsys_access_modeBC(Memsys *sys, Addr lineaddr, Access_Type type,uns core_id){
  ...
  if(needs_dcache_access)
  {
    Flag temp= cache_access(sys->dcache, lineaddr, is_write, core_id);

    if(temp==MISS)
    {
      cache_install(sys->dcache, lineaddr, is_write, core_id);
      delay = delay+ memsys_L2_access(sys, lineaddr, 0, core_id);

      if(sys->dcache->last_evicted_line.dirty && sys->dcache->last_evicted_line.valid)
      {
        var= (sys->dcache->num_sets-1) & lineaddr;
        evicted= sys->dcache->last_evicted_line.tag * sys->dcache->num_sets + var;
        del= del+ memsys_L2_access(sys, evicted, 1, core_id);
  ...
  {
    Flag temp=cache_access(sys->icache, lineaddr, is_write, core_id);
    if(temp==MISS)
    {
      cache_install(sys->icache, lineaddr, is_write, core_id);
      delay = delay+ memsys_L2_access(sys, lineaddr, is_write, core_id);
    }
  }
  return delay;
}
...
uns64   memsys_L2_access(Memsys *sys, Addr lineaddr, Flag is_writeback, uns core_id){
  ...
  Flag temp= cache_access(sys->l2cache, lineaddr, is_writeback, core_id);
  ...
  if( temp!= HIT )
  {
    ...
    if((sys->l2cache->last_evicted_line.dirty) && (sys->l2cache->last_evicted_line.valid)){
                
                var = lineaddr & (sys->l2cache->num_sets-1);
                evicted = sys->l2cache->last_evicted_line.tag * sys->l2cache->num_sets + var;
                
                del= del + dram_access(sys->dram, evicted, 1); 

...
uns64   dram_access_sim_rowbuf(DRAM *dram,Addr lineaddr, Flag is_dram_write){
  ...
  uns64 row_index = lineaddr / (DRAM_BANKS * ROWBUF_SIZE / CACHE_LINESIZE);
  
  if(rows[bank_index] == row_index) {   //hit
    delay = DRAM_T_CAS + DRAM_T_BUS;
  }
  else if(rows[bank_index] == -1) {    //miss
    rows[bank_index] = row_index;
    delay = DRAM_T_ACT + DRAM_T_CAS + DRAM_T_BUS;   
  }
  else {
    rows[bank_index] = row_index;
    delay = DRAM_T_PRE + DRAM_T_ACT + DRAM_T_CAS + DRAM_T_BUS;  //conflict
  }
